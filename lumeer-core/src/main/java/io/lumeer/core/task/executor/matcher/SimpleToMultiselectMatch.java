...
public class SimpleToMultiselectMatch implements MatchQueryProvider {

   private final DocumentMatcher matcher;

   SimpleToMultiselectMatch() {
      matcher = null;
   }

   public SimpleToMultiselectMatch(final DocumentMatcher matcher) {
   ...
   public Query getMatchQueryForRemoval(final Object oldValue, final Object newValue) {
      return new Query(Collections.singletonList(
                  new QueryStem(
                        ...
                        List.of(
                              new CollectionAttributeFilter(matcher.getThatCollection().getId(), matcher.getThatAttribute().getId(), ConditionType.HAS_SOME, List.of(new ConditionValue(oldValue)))
                        ),
   ...
   public Query getMatchQueryForCreation(final Object oldValue, final Object newValue) {
      return new Query(Collections.singletonList(
            new QueryStem(
                  ...
                  Collections.emptySet(),
                  List.of(
                        new CollectionAttributeFilter(matcher.getThatCollection().getId(), matcher.getThatAttribute().getId(), ConditionType.HAS_SOME, List.of(new ConditionValue(newValue)))
                  ),
                  Collections.emptyList()
            )
      )
   ...
   public Query getMatchQuery(final Collection collection, final Attribute attribute, final Object newValue) {
      return new Query(Collections.singletonList(
            new QueryStem(
                  ...
                  Collections.emptySet(),
                  List.of(
                        new CollectionAttributeFilter(collection.getId(), attribute.getId(), ConditionType.HAS_SOME, List.of(new ConditionValue(newValue)))
                  ),
                  Collections.emptyList()
            )
      )
