...
class JvmArrayProxy(var values: Array<Any?>, val locale: Locale = Locale.getDefault()) : ProxyArray {

    override fun get(index: Long): Any? {
        checkIndex(index)
        val idx = index.toInt()
        return if (values.getOrNull(idx) != null) JvmObjectProxy.encodeObject(values[idx]!!, locale)
        else null
    }
